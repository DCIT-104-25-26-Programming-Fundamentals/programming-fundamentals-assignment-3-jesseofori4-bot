...
function main() {
  while (true) {
    displayMenu();
    const choice = parseInt(readlineSync.question('Select an operation (1-7): '), 10);
    if (choice === 7) {
      console.log('Goodbye!');
      break;
    }
    ...
    switch (choice) {
      ...
      case 4:
        ...
        if (divisionResult !== null) {
          console.log(`Result: ${num1} / ${num2} = ${divisionResult.toFixed(2)}`);
        }
        break;
      case 5:
        const modulusResult = modulus(num1, num2);
        if (modulusResult !== null) {
          console.log(`Result: ${num1} % ${num2} = ${modulusResult.toFixed(2)}`);
        }
        break;
      case 6:
